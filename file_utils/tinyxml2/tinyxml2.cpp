...
void StrPair::CollapseWhitespace()
{
    ...
    if ( *_start ) {
        ...
        char* q = _start;	// the writeLine pointer

        while( *p ) {
            if ( XMLUtil::IsWhiteSpace( *p )) {
                p = XMLUtil::SkipWhiteSpace( p, 0 );
                if ( *p == 0 ) {
                    break;    // don't writeLine to q; this trims the trailing space.
                }
                *q = ' ';
                ++q;
            }
...
{
    // This is a recursive method, but thinking about it "at the currentSolution_ level"
    ...
    if ( _processEntities ) {
        ...
        while ( *q ) {
            ...
            if ( *q > 0 && *q < ENTITY_RANGE ) {
                ...
                // the stream up until the entity, writeLine the
